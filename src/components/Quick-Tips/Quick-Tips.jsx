...
const handleIconClick = async (index) => {
  const tip = tipsList[index];
  console.log("tip selected:", tip); // للتأكيد
    ...
    >
      ...
      <div className="flex flex-col justify-center items-center mt-44">
        <div className="bg-[#D9D9D9] rounded-lg shadow-md max-w-md relative flex flex-col justify-between min-h-[60px] px-4 pt-8 pb-4">
          ...
          <Link
  to={`/howto/${selectedTip.category}?highlighted=${selectedTip.id}`}
              className={`${
              gender === "male"
                ? "bg-[#88c1ec]"
                : gender === "female"
...
// const fetchAdvice = async (apiUrl) => {
//   try {
...
//     const content =
//       i18n.language === "ar" ? data.advice_mather_ar : data.advice_mather_en;

//     return { id: data.id, text: content }; // <-- رجع الـ ID كمان
//   } catch (error) {
//     console.error("Error fetching advice:", error);
//     return { id: null, text: "" };
//   }
...
//   const handleIconClick = async (index) => {
//   const tip = tipsList[index];
//   const { id: adviceId, text: newText } = await fetchAdvice(tip.api);
